...
<template>
  <section class="flex flex-col gap-3">
    <header class="flex items-baseline justify-between gap-2">
      <h1 class="text-lg font-semibold tg-text">{{ t("online.title") }}</h1>
    ...
    <ul v-else class="flex flex-col gap-2">
      <li
        ...
        class="rounded-xl px-3 py-3 tg-card"
      >
        <div class="flex items-center justify-between gap-2">
          ...
          <div class="flex shrink-0 gap-2">
            ...
            <NuxtLink
              ...
              class="rounded-lg px-2 py-1 text-xs font-medium tg-button"
            >
              {{ t("details.status") }}
            </NuxtLink>
          </div>
        </div>
        <dl v-if="follow.live && follow.live.status === 'online'" class="mt-2 space-y-0.5 text-xs tg-hint">
