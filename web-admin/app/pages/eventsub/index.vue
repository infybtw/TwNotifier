<template>
  <div>
    ...
    <DataTable :columns="columns" :rows="subs ?? []" :loading="status === 'pending'" />
  </div>
</template>
...
async function handleReload() {
  ...
  try {
    const res = await post<any>('/eventsub/reload', {})
...
async function handleDisconnect() {
  if (!confirm('Delete ALL EventSub subscriptions?')) return
  loading.value = true; message.value = ''; isError.value = false
  try {
    const res = await post<any>('/eventsub/disconnect', {})
    message.value = `Deleted ${res.deleted} subscriptions`
    refresh()
...
async function handleCleanup() {
  loading.value = true; message.value = ''; isError.value = false
  try {
    const res = await post<any>('/eventsub/cleanup', {})
