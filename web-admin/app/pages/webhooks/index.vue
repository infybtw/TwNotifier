<template>
  <div>
    ...
    <DataTable :columns="columns" :rows="webhooks ?? []" :loading="status === 'pending'" @row-click="openDetail" />

    <Modal :open="showDetail" title="Webhook Details" @close="showDetail = false">
...
const columns = [
  ...
  { key: 'created_at', label: 'Created', truncate: 10 },
]

function openDetail(row: any) { selectedRow.value = row; showDetail.value = true }
async function handleReload() { loading.value = true; message.value = ''; isError.value = false; try { const res = await post<any>('/webhooks/reload', {}); message.value = `Reloaded: ${res.before} → ${res.after}`; refresh() } catch { message.value = 'Failed to reload'; isError.value = true } finally { loading.value = false } }
async function handleDisconnect() { if (!confirm('Delete ALL webhook subscriptions?')) return; loading.value = true; message.value = ''; isError.value = false; try { const res = await post<any>('/webhooks/disconnect', {}); message.value = `Deleted ${res.deleted} webhooks`; refresh() } catch { message.value = 'Failed to disconnect'; isError.value = true } finally { loading.value = false } }
async function handleCleanup() { loading.value = true; message.value = ''; isError.value = false; try { const res = await post<any>('/webhooks/cleanup', {}); message.value = `Total: ${res.total}, Removed: ${res.removed}, Remaining: ${res.remaining}`; refresh() } catch { message.value = 'Failed to cleanup'; isError.value = true } finally { loading.value = false } }
</script>
